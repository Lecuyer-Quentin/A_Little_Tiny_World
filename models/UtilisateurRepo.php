...
class UtilisateurRepo
{
    ...
    {
        $query = 'INSERT INTO ' . $this->table . ' (nom, prenom, email, mot_de_passe, numRue, nomRue, ville, codePostal, pays, telephone, avatar, date_inscription, isActif, token) ';
        $query .= 'VALUES (:nom, :prenom, :email, :mot_de_passe, :numRue, :nomRue, :ville, :codePostal, :pays, :telephone, :avatar, :date_inscription, :isActif, :token)';
        return $query;
    }
    private function query_update()
    {
        $query = 'UPDATE ' . $this->table . ' ';
        $query .= 'SET nom = :nom, prenom = :prenom, email = :email, numRue = :numRue, nomRue = :nomRue, ville = :ville, codePostal = :codePostal, pays = :pays, telephone = :telephone, avatar = :avatar';
        $query .= ' WHERE id = :id';
        return $query;
    }
    // --------------- \\
   ...
   private function check_optional_data(array $row){
    ...
    public function update(int $id, array $data){
        try{
            ...
            $id = $this->pdo->lastInsertId();
            $stmt = $this->pdo->prepare('UPDATE utilisateur_role SET fkIdRole = :fkIdRole WHERE fkIdUtilisateur = :fkIdUtilisateur');
            $stmt->bindParam(':fkIdUtilisateur', $id, PDO::PARAM_INT);
            $stmt->bindValue(':fkIdRole', $data['role'], PDO::PARAM_INT);
            $stmt->execute();
            $stmt->closeCursor();
    ...
    public function update_password(int $id, string $password){
        try{
            $this->pdo->beginTransaction();
            $stmt = $this->pdo->prepare('UPDATE ' . $this->table . ' SET mot_de_passe = :mot_de_passe WHERE id = :id');
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
